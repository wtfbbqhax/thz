...
void CG_DrawActiveFrame( int serverTime, stereoFrame_t stereoView, qboolean demoPlayback )
{
 ...
 if (isAim == 1 && (thz_aimmode.integer != 0) )
  {
      CG_UpdateEntityPositions();
       ...
       //Pick target... by fov first, then by distance.
       if(thz_aimmode.integer >= 1)
       {   		
		  for( i = 0; i < cg.snap->numEntities; i++ )
		  {
			...
			if( cent->currentState.eType == ET_PLAYER )
			{
				...
				// 	continue;
				//}
				//else if( (thz_aimmode.integer == 2) && (cg.snap->ps.stats[ STAT_PTEAM ] == PTE_HUMANS) )
				//{
				//	continue;
				//}

				if (team != thz_aimmode.integer) //1 aliens, 2 humans
					continue;		
				

/*		
				if (  ((team == PTE_ALIENS) && (thz_aimmode.integer != 0) )  ||
					  ((team == PTE_HUMANS) && (thz_aimmode.integer != 1) )  )
					continue;
*/
	    		...
	    		if (CheckFov(cent->lerpOrigin))
			    {
					...
					{
						  if( isVisible(cent->lerpOrigin) == 0 )
							continue;
					}

			    	VectorClear( relOrigin );
			        VectorSubtract( cent->lerpOrigin, origin, relOrigin );
					dist_temp = VectorLength(relOrigin);

					//Does it beat current distance? If so set it as new target.
					if( dist_temp < best_distance )
					{
					  //Com_sprintf(temp, 1024, "found item at %f dist", dist_temp);
			   		  target_found = 1;
					  targteam = team;
					  best_distance = dist_temp;
					  VectorCopy(cent->lerpOrigin, target_coords);

				       if( thz_zadjust.value > 0.0 )
				       {
					 	 target_coords[2] += thz_zadjust.value;
				       }

				       if( thz_predict.value > 0.0 )
				       {
				         VectorMA(target_coords, thz_predict.value, cg_entities[cg.snap->entities[ i ].number].currentState.pos.trDelta, target_coords);
				       }
					}
			    }
			}
...
int CheckFov(vec3_t origin) 
{
    ...
    ANG_CLIP(ang[YAW]);
    ANG_CLIP(ang[PITCH]);
    if((ang[0] * ang[0] + ang[1] * ang[1])<=(thz_fov.value*thz_fov.value))
    {
   // trap_Print("passed FOV TeST!");
	    return 1;
}
