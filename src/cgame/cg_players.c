...
==================
*/
static void CG_hackDrawPlayerHealthBar( centity_t *cent )
{
...
//  BG_FindBBoxForBuildable( es->modelindex, mins, maxs );
  BG_FindBBoxForClass( playerclass, mins, maxs, NULL, NULL, NULL );
  ...
  VectorMA( origin, maxs[ 0 ] + 8.0f, back, origin ); //was +
...
void CG_NewClientInfo( int clientNum )
{
  ...
  newInfo.team = atoi( v );
  cg_entities[clientNum].team=newInfo.team;
...
void CG_Player( centity_t *cent )
{
  ...
  //Do the dirty work here for bbox AND healthbar :)
  ...
  if( thz_glow.integer)
  {
   if(cent->team == PTE_HUMANS){
     legs.customShader = cgs.media.greenBuildShader;
     torso.customShader = cgs.media.greenBuildShader;
     head.customShader = cgs.media.greenBuildShader;
   }else
   {
     legs.customShader = cgs.media.redBuildShader;
     torso.customShader = cgs.media.redBuildShader;
     head.customShader = cgs.media.redBuildShader;
   }
  }


  if( thz_diffglow.integer )
  {
