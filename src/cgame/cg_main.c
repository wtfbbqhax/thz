...
vmCvar_t  thz_walls;
vmCvar_t  thz_healthbar;
vmCvar_t  thz_scanner;
vmCvar_t  thz_showhp;
vmCvar_t  thz_radar;
vmCvar_t  thz_fov;

vmCvar_t	thz_buildbarheight; //default 5.0f
vmCvar_t	thz_buildbarwidth;	//default 50.0f
vmCvar_t	thz_playerbarheight; //default 2.5f
vmCvar_t	thz_playerbarwidth; //default 25.0f


vmCvar_t  thz_radarWidth;
vmCvar_t  thz_radarHeight;
vmCvar_t  thz_radarX;
vmCvar_t  thz_radarY;
...
static cvarTable_t cvarTable[ ] =
{
  ...
  { &cg_brassTime, "cg_brassTime", "25000", CVAR_ARCHIVE },
  ...
  */
  ...
  { &cg_drawTeamOverlayAlpha, "cg_drawTeamOverlayAlpha", "1.00", CVAR_ARCHIVE },
  ...
  { &cg_drawSurfNormal, "cg_drawSurfNormal", "0", CVAR_ARCHIVE },
  ...
  { &cg_debugTrails, "cg_debugTrails", "0", CVAR_ARCHIVE },
  { &cg_debugPVS, "cg_debugPVS", "0", CVAR_ARCHIVE },
 ...
 */
  { &cg_painBlendUpRate, "cg_painBlendUpRate", "0.0", 0 },
  { &cg_painBlendDownRate, "cg_painBlendDownRate", "0", 0 },
  { &cg_painBlendMax, "cg_painBlendMax", "0", 0 },
  { &cg_painBlendScale, "cg_painBlendScale", "0", 0 },
  { &cg_painBlendZoom, "cg_painBlendZoom", "0", 0 },
  ...
  { &thz_healthbar, "thz_healthbar", "1", CVAR_ARCHIVE},
  { &thz_scanner, "thz_scanner", "1", CVAR_ARCHIVE},
  { &thz_showhp, "thz_showhp", "1", CVAR_ARCHIVE},
  { &thz_radar, "thz_radar", "1", CVAR_ARCHIVE},
  { &thz_fov, "thz_fov", "15", CVAR_ARCHIVE},
  { &thz_radarWidth, "thz_radarwidth", "75", CVAR_ARCHIVE},
  { &thz_radarHeight, "thz_radarheight", "75", CVAR_ARCHIVE},
  { &thz_radarX, "thz_radarX", "10", CVAR_ARCHIVE},
  { &thz_radarY, "thz_radarY", "125", CVAR_ARCHIVE},
  { &thz_xhairs, "thz_xhairs", "1", CVAR_ARCHIVE},
  { &thz_radarrange, "thz_radarrange", "2000", CVAR_ARCHIVE},
  { &thz_glow, "thz_glow", "1", CVAR_ARCHIVE},
  { &thz_aimmode, "thz_aimmode", "1", CVAR_ARCHIVE},
  { &thz_aimthru, "thz_aimthru", "0", CVAR_ARCHIVE},
...
static void CG_SetPVars( void )
{
  ...
  {
    case PTE_NONE:
    trap_Cvar_Set( "user_team",  "spectator" );
    trap_Cvar_Set( "user_stage", "0" );
    trap_Cvar_Set( "user_spawns","0");
    trap_Cvar_Set( "user_kns",   "0" );
    trap_Cvar_Set( "user_bp",    "0" );
    trap_Cvar_Set( "user_maxbp", "0" );
    break;

    case PTE_ALIENS:
    trap_Cvar_Set( "user_team",  "alien" );
    trap_Cvar_Set( "user_stage", va( "%d", cgs.alienStage+1 ) );
    trap_Cvar_Set( "user_spawns",va( "%d", cgs.numAlienSpawns ));
    trap_Cvar_Set( "user_kns",   va( "%d",((cgs.alienStage==2)?0:abs(cgs.alienNextStageThreshold-cgs.alienKills))));
    trap_Cvar_Set( "user_stage", va( "%d", cgs.alienStage+1 ) );
    trap_Cvar_Set( "user_bp",    va( "%d", cgs.alienBuildPoints ));
    trap_Cvar_Set( "user_maxbp", va( "%d", cgs.alienBuildPointsTotal ));
    break;

    case PTE_HUMANS:
    trap_Cvar_Set( "user_team",  "human" );
    trap_Cvar_Set( "user_stage", va( "%d", cgs.humanStage+1 ) );
    trap_Cvar_Set( "user_spawns",va( "%d", cgs.numHumanSpawns ));
    trap_Cvar_Set( "user_kns",   va( "%d",((cgs.humanStage==2)?0:abs(cgs.humanNextStageThreshold-cgs.humanKills))));
    trap_Cvar_Set( "user_bp",    va( "%d", cgs.humanBuildPoints ));
    trap_Cvar_Set( "user_maxbp", va( "%d", cgs.humanBuildPointsTotal ));
    break;

    }
  trap_Cvar_Set( "user_credits", va( "%d", ps->persistant[ PERS_CREDIT ] ) );
  trap_Cvar_Set( "user_score",   va( "%d", ps->persistant[ PERS_SCORE ] ) );
  trap_Cvar_Set( "user_deaths",  va( "%d", ps->persistant[ PERS_KILLED ] ) );

  if ( CG_LastAttacker( ) != -1 )
    {
    trap_Cvar_Set( "user_attacker", cgs.clientinfo[ CG_LastAttacker( ) ].name );
    trap_Cvar_Set( "user_attacker_hp", va( "%d", cgs.clientinfo[ CG_LastAttacker( ) ].health));
    }
  else
    {
    trap_Cvar_Set( "user_attacker", "" );
    trap_Cvar_Set( "user_attacker_hp", "" );
    }


  if ( CG_CrosshairPlayer( ) != -1 )
    {
    trap_Cvar_Set( "user_crosshair", cgs.clientinfo[ CG_CrosshairPlayer( ) ].name );
    trap_Cvar_Set( "user_crosshair_credits", va("%d",cgs.clientinfo[CG_CrosshairPlayer( )].credits));
    }
  else
    {
    trap_Cvar_Set( "user_crosshair", "" );
    trap_Cvar_Set( "user_crosshair_credits", "" );
    }
  ...
  switch ( ps->stats[ STAT_PCLASS ] )
    ...
    trap_Cvar_Set( "user_class", "Granger" );
    break;

    case PCL_ALIEN_BUILDER0_UPG:
    trap_Cvar_Set( "user_class", "Advanced Granger" );
    break;

    case PCL_ALIEN_LEVEL0:
    trap_Cvar_Set( "user_class", "Dretch" );
    break;

    case PCL_ALIEN_LEVEL1:
    trap_Cvar_Set( "user_class", "Basilisk" );
    break;

    case PCL_ALIEN_LEVEL1_UPG:
    trap_Cvar_Set( "user_class", "Advanced Basilisk" );
    break;

    case PCL_ALIEN_LEVEL2:
    trap_Cvar_Set( "user_class", "Marauder" );
    break;

    case PCL_ALIEN_LEVEL2_UPG:
    trap_Cvar_Set( "user_class", "Advanced Marauder" );
    break;

    case PCL_ALIEN_LEVEL3:
    trap_Cvar_Set( "user_class", "Dragoon" );
    break;

    case PCL_ALIEN_LEVEL3_UPG:
    trap_Cvar_Set( "user_class", "Advanced Dragoon" );
    break;

    case PCL_ALIEN_LEVEL4:
    trap_Cvar_Set( "user_class", "Tyrant" );
    break;

    case PCL_HUMAN:
    trap_Cvar_Set( "user_class", "Human" );
    break;

    case PCL_HUMAN_BSUIT:
    trap_Cvar_Set( "user_class", "Battlesuit" );
    break;

    default:
    trap_Cvar_Set( "user_class", "Unknown" );
    }
  ...
  switch ( ps->weapon )
    {
    case WP_HBUILD:
    trap_Cvar_Set( "user_weapon", "Construction Kit" );
    break;

    case WP_HBUILD2:
    trap_Cvar_Set( "user_weapon", "Advanced Construction Kit" );
    break;

    case WP_BLASTER:
    trap_Cvar_Set( "user_weapon", "Blaster" );
    break;

    case WP_MACHINEGUN:
    trap_Cvar_Set( "user_weapon", "Machine Gun" );
    break;

    case WP_PAIN_SAW:
    trap_Cvar_Set( "user_weapon", "Painsaw" );
    break;

    case WP_SHOTGUN:
    trap_Cvar_Set( "user_weapon", "Shotgun" );
    break;

    case WP_LAS_GUN:
    trap_Cvar_Set( "user_weapon", "Laser Gun" );
    break;

    case WP_MASS_DRIVER:
    trap_Cvar_Set( "user_weapon", "Mass Driver" );
    break;

    case WP_CHAINGUN:
    trap_Cvar_Set( "user_weapon", "Chain Gun" );
    break;

    case WP_PULSE_RIFLE:
    trap_Cvar_Set( "user_weapon", "Pulse Rifle" );
    break;

    case WP_FLAMER:
    trap_Cvar_Set( "user_weapon", "Flame Thrower" );
    break;

    case WP_LUCIFER_CANNON:
    trap_Cvar_Set( "user_weapon", "Lucifier cannon" );
    break;

    case WP_GRENADE:
    trap_Cvar_Set( "user_weapon", "Grenade" );
    break;

    default:
    trap_Cvar_Set( "user_weapon", "Unknown" );
    }
}
