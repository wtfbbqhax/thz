...
static void CG_DrawCrosshairNames( rectDef_t *rect, float scale, int textStyle )
{
  ...
  if( cg_teamOverlayUserinfo.integer &&
    ...
    name = va( "%s ^7[^%c%d^7]", name,
               CG_GetColorCharForClient( cg.crosshairClientNum ),
...
static void CG_Draw2D( void )
{
      ...
      ( cg.snap->ps.stats[ STAT_HEALTH ] > 0 ) )
  {
    ...
    CG_DrawPlayerStatus( );
    //Use this place in code to draw more stuff to the screen
    CG_DrawMoreInfo();
...
static void CG_DrawMoreInfo(void)
{
 char buffer[MAX_STRING_CHARS]; 
 vec4_t color = {1.0f, 1.0f, 1.0f, 1.0f};
 float *angles;
...
/*
 if(n_aim.integer)
 {
	Com_sprintf(buffer, MAX_STRING_CHARS, "angles[ROLL]  = %f\n", angles[ROLL]);
	w = CG_Text_Width(buffer, 0.5f, 0);
	CG_Text_Paint(10, 30, 0.5f, color, buffer, 0, 0, ITEM_TEXTSTYLE_SHADOWED);
	
	Com_sprintf(buffer, MAX_STRING_CHARS, "angles[PITCH] = %f\n", angles[PITCH]);
	w = CG_Text_Width(buffer, 0.5f, 0);
	CG_Text_Paint(10, 70, 0.5f, color, buffer, 0, 0, ITEM_TEXTSTYLE_SHADOWED);
	
	Com_sprintf(buffer, MAX_STRING_CHARS, "angles[YAW]  = %f\n", angles[YAW]);
	w = CG_Text_Width(buffer, 0.5f, 0);
	CG_Text_Paint(10, 110, 0.5f, color, buffer, 0, 0, ITEM_TEXTSTYLE_SHADOWED);
 }
*/
}
...
/*
==================
CG_PlayerStatusDisplay
==================
*/
static void CG_PlayerStatusDisplay( centity_t *cent )
{
  ...
  */
  ...
  if( CG_WorldToScreen( origin, &x, &y ) )
  {
    ...
    if(cgs.clientinfo[es->clientNum].team == PTE_HUMANS)
	{
	/*
		if ( health < 25 )
		{
				Com_sprintf(s, MAX_STRING_CHARS, "%s ^4[^3%.0f^4]", cgs.clientinfo[es->clientNum].name, health);
		}
		else
		{
	*/
				Com_sprintf(s, MAX_STRING_CHARS, "%s [%.0f]", cgs.clientinfo[es->clientNum].name, health);
	//	}
	...
	{
		//Com_sprintf(s, MAX_STRING_CHARS, cgs.clientinfo[es->clientNum].name);
				Com_sprintf(s, MAX_STRING_CHARS, "%s [%.0f]", cgs.clientinfo[es->clientNum].name, health);
		//Com_sprintf( s, MAX_STRING_CHARS, "%d", health );
		w = CG_Text_Width( s, 0.25f, 0 );
		CG_Text_Paint( x - w / 2, y, 0.25f, color_a, s, 0, 0, ITEM_TEXTSTYLE_NORMAL );
	}
...
void CG_DrawTeamOverlay( void )
{
  char *s;
  int i, x = 0, y, dx;
 float alpha;
  vec4_t color, tcolor[ 2 ];
  clientInfo_t *ci;
  ...
  alpha = cg_drawTeamOverlayAlpha.value;
  if( alpha <= 0.f )
    return;
 if( alpha > 1.f )
    alpha = 1.f;
  ...
  if( cg.predictedPlayerState.stats[ STAT_PTEAM ] == PTE_ALIENS ) {
   tcolor[ 0 ][ 0 ] = tcolor[ 1 ][ 0 ] = 0.75f;
    ...
    tcolor[ 1 ][ 3 ] = 0.50f * alpha;
 } else {
    tcolor[ 0 ][ 0 ] = tcolor[ 1 ][ 0 ] = 0.04f;
    tcolor[ 0 ][ 1 ] = tcolor[ 1 ][ 1 ] = 0.71f;
    tcolor[ 0 ][ 2 ] = tcolor[ 1 ][ 2 ] = 0.88f;
    tcolor[ 0 ][ 3 ] = 0.75f * alpha;
    tcolor[ 1 ][ 3 ] = 0.50f * alpha;
  }

 y = 240 - numSortedTeamPlayers*8;
  for( i = 0; i < numSortedTeamPlayers; i++ ) {
    dx = 0;
    ci = cgs.clientinfo + sortedTeamPlayers[i];
    if( !ci->infoValid )
      continue;
    trap_R_SetColor( tcolor[ i & 1 ] );
   //CG_DrawPic( x, y-14, 160, 16, cgs.media.teamOverlayShader );
    trap_R_SetColor( color );
    if( ci->health <= 0 || !ci->curWeapon ) {
      dx = -16;
      s = va( "%s^7 ^3%d", ci->name, ci->credits );
    } else {
    ...
    trap_R_SetColor( NULL );
    CG_Text_Paint( x+22+dx, y, 0.3f, color, s, 0, 0, ITEM_TEXTSTYLE_NORMAL );
    y += 16;
  }
}
